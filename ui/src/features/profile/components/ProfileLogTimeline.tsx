...
export function ProfileLogTimeline({ logs, loading, isOwnProfile }: ProfileLogTimelineProps) {
  if (loading && logs.length === 0) {
    return (
      <div className="space-y-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="rounded-glass border border-border bg-white p-5 animate-pulse">
  ...
  if (!loading && logs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-border bg-chalk py-20 text-center">
        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-border bg-white">
  ...
  return (
    <motion.div variants={staggerContainer} initial="initial" animate="animate" className="relative pl-5">
      <div className="absolute bottom-4 left-1.5 top-4 w-px bg-gradient-to-b from-accent/60 via-border to-transparent" />
      <div className="space-y-4">
        {logs.map((log, i) => (
          <motion.article key={log.id} variants={fadeUp} custom={i} className="relative">
            <div className="absolute -left-[25px] top-6 h-3 w-3 rounded-full border border-accent/50 bg-white shadow-glow" />
            <Link
              to={`/p/${log.project.id}/logs/${log.id}`}
              className="group block rounded-[20px] border border-border bg-white p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-accent/30 hover:bg-chalk hover:shadow-glow"
            >
              <div className="mb-3 flex flex-wrap items-center gap-2 text-caption text-ink-tertiary">
                ...
                <span className="rounded-pill border border-border bg-chalk px-2 py-0.5">
                  {log.project.title}
                </span>
